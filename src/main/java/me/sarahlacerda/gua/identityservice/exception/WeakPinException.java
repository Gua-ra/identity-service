 ...
 *
 * <p>
 * Extends {@link InvalidPinException} so existing handling that catches
 * invalid PINs keeps working, while allowing the API layer to surface a
 * distinct
 * {@code weak_pin} error code (separate from the {@code invalid_pin} used when
 * a
