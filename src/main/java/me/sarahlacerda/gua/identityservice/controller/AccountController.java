...
public class AccountController {
        ...
        @Operation(summary = "Start a phone-number change", description = "Spends a PHONE_CHANGE-scoped reauth token and a mandatory step-up factor (account PIN when set, and/or a user-verifying passkey assertion from /security/passkey/stepup/options), then sends an OTP to the NEW number and returns a challenge to redeem at /account/phone/change/complete. Accounts with neither a PIN nor a passkey receive step_up_required and must set up two-step verification first. The OLD number is alerted out of band.", security = @SecurityRequirement(name = "oidcAccessToken"))
        @ApiResponses({
