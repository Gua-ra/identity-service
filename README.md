...
**The sheet returns to the app that opened it.** On completion the session echoes back an app-scheme redirect, and each build of the apps registers its own: the store build answers `global.gua`, a QA build `global.gua.dev`, an Android debug build `global.gua.debug`. So the redirect is the first one registered by the **caller's own OIDC client**, read off the audience of the bearer token this service already verified before accepting it. One value for the whole deployment left the sheet on a QA build handing off to a scheme that build does not answer, so it never dismissed itself, and on a phone that also has the store build installed the completion went to the wrong app. `idp.login.enroll.redirect-uri` (`IDP_LOGIN_ENROLL_REDIRECT_URI`) stays as the fallback for a token that names no client of ours, which is every homeserver-issued one, and for a client that registered no redirect. The caller never supplies a redirect and cannot name the client: a bearer endpoint that took one would hand a session's completion wherever the caller asked.
