...
| Method & path | Auth | Purpose |
| --- | --- | --- |
| `GET /security/pin/status` | Bearer | Whether the user has a PIN set (drives the "set up two-step verification" nudge), plus `changePhoneCooldownRemainingSeconds`: how long the fresh-2FA hold below still has to run. |
...
**Phone changes require two-step verification.** Because the reauth OTP goes to the *current* number, which a SIM-swap attacker may control, `/account/phone/change/start` additionally demands a non-phone factor: the account PIN (always, when one is set) and/or a user-verifying passkey assertion from `/security/passkey/stepup/options`. Accounts with **neither** a PIN nor a passkey are hard-blocked with `403 step_up_required` and must set up two-step verification (a PIN via `/security/pin`, or a passkey) before they can change their number. There is no token-only fallback. Two further refusals sit on top, neither of which replaces anything: a passkey that did not verify the user is not accepted as the factor (`403 passkey_user_verification_required`), and a PIN that was created, changed or reset inside the fresh-2FA hold is not accepted either (`400 twofa_cooldown_active`, see [Account PIN](#account-pin-two-step-verification)).
