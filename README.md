...
**One head, one order, no races.** Exactly one `account_authority_head` row per account, read `FOR UPDATE` by every writer, and acceptance is a compare-and-set on `prevHash` and `seq`. Two devices acting at once produce one winner and one refusal carrying the current head. A record inside its opposition window has already taken its `seq`, so an immediate transition cannot starve a delayed one; a **cancelled** record gives that slot back, to the position its own `prevHash` names, because keeping it left an account whose first adoption was opposed unable to ever adopt again and reported as `AUTHORITY_LOST` without ever having been rooted. The retry then lands at the `seq` both clients build it for. Competing transitions resolve by ADM-002 D2's rank, not by a freeze: rank 2 is a recovery signed by the committed recovery authority key, rank 1 any record signed by an active device, rank 0 a recovery authorized through account recovery. A higher rank cancels a pending lower one, an equal rank is refused, and each cancelled initiation doubles the backoff of the key set that opened it, charged once the submission that cancelled it is accepted rather than before. The cooldown a cancellation writes is recorded with the shape it was written for, so it refuses another transition of that shape and not every other one the owner might need.

**Objecting takes an object.** `GUAO` Oppose names the record it objects to by its SHA-256 and is signed by a key the chain has active and unquarantined at that moment. It **takes no slot and starts no window**: it cancels the record it names, or it is refused, and it never joins the chain, so it stands at the same `seq` and `prevHash` as the record it cancels. This is the claim a bearer session cannot make. A session may object to an **adoption**, where the account holds no authority to weigh, and to the weaker recovery path; accepting its word for a grant or a revocation would let a stolen session veto the owner's own revocation of the thief's device. No factor is asked for and no hold is weighed on either shape of objection: the holds gate starting a transition, never opposing one.

**Adding a device takes a candidate.** The new device generates its own key, never receives another device's, and offers the public half as a candidate under its own session; the server hands back an eight-character fingerprint from `ABCDEFGHJKLMNPQRSTUVWXYZ2346789` (no I, O, 0, 1, 5 or S, because a fingerprint gets read aloud). The fingerprint is **derived from the key**, so both phones compute the same characters from the same 32 bytes and a person comparing them learns the two are looking at one key; a server-issued nonce would only show that both had spoken to the same server. The granting device reads the account's candidates, shows the fingerprint, and signs a grant over the one the user confirms. A grant over a key that is not a live candidate of that same account is refused (`authority_unknown_candidate`), a candidate lives ten minutes, and its grant spends it, so a second grant over the same key needs a fresh comparison.

**The device set.** A per-device key, never one key copied to every device: copying makes revocation meaningless, since the revoked device still holds the key the account is defined by. A grant takes effect on acceptance, because it only adds, and its holder is **quarantined** for one window: it may not sign a grant, a revocation or an approval, and it does not count toward the active device a revocation must leave behind. That quarantine is the grant's opposition window: an `Oppose` from any other active device, naming the grant, **revokes the granted device at once**, and the grant record stays in the chain because every later `prevHash` covers it. A grant is therefore also refused on an account nothing can tell about it, like every other transition that has to be announced. Revoking another device waits out the window; revoking itself is immediate, because a device removing its own authority reduces what an attacker holding it could do. Neither may leave the account with no unquarantined active device.

**The browser holds no authority, ever.** A browser login grants account access and never enters the device set, and this is a rule rather than a default: there is no flag that lets a web session sign an authority record. Authority-sensitive actions reachable from the web create a pending approval carrying the account, the digest the server derives from the named action, and a challenge; the digest is never taken from the caller, because the sentence the device shows the reader and the bytes it signs have to be the same action; the browser shows a four-character code from an alphabet with no look-alikes, and an active authority device shows the same code and the action in the reader's own words and signs it. A malicious page reaches the approval and not the signature.

**What holds that, and what does not.** The native-session rule can only read the client id from a token's verified audience, and both apps authenticate with MAS-issued tokens that this service validates through the homeserver's whoami: those name no client of ours at all. So the rule cannot tell an app from a page, and it says so in its own javadoc; it refuses a registered client of ours that is not listed as native, and accepts a token that names none, because refusing that refused every real phone on every deployment. What keeps a page out is the two things a page cannot do anything about: every authority endpoint is bearer-only, and the web step-up sheet holds a Redis login session and its CSRF cookie rather than an access token, so a page holding that session gets a 401; and the key a record is signed with is generated in the platform keychain or keystore, non-synced, with no channel to a page. `AccountAuthorityGuardTest` pins the first.
...
**Endpoints** (all bearer, all `503` while the flag is off; the four transitions additionally refuse a registered client of ours that this deployment has not listed as native):
...
| `POST /account/authority/oppose` | Objects to the pending transition on the session alone. Free the first time, a step-up on any factor at any age after that. Accepted for an adoption and for the weaker recovery path only. |
| `POST /account/authority/oppose/record` | Objects with a signed `Oppose`, from a device the chain has active and unquarantined. This is what "an active device may oppose" means for a grant, a revocation and a recovery. |
...
| `POST /account/authority/device/grant` | Activates another device key, effective at once, grantee quarantined and the grant opposable for that window. |
...
| `POST /security/authority/step-up/start` | Mints the one-time URL of a web step-up scoped to one purpose, for a platform that cannot run the assertion natively. Native session only, and refused for a purpose that asks for no factor or an account that holds neither a passkey this deployment can assert nor a PIN. |
| `POST /login/authority/stepup/passkey/options`, `POST /login/authority/stepup/passkey/verify`, `POST /login/authority/stepup/pin` | The sheet's own steps, on the login cookie rather than a bearer. Two arms, and deliberately no third: no code is sent to the account's number at any step of this page. |

**The step-up can run in a web sheet**, because it cannot always run natively. The policy is a user-verifying passkey assertion, or the PIN where policy allows it, and never a code sent to the number. One client platform cannot produce a WebAuthn assertion at all, so on that platform the policy would collapse to PIN-only and an account that correctly chose passkey-only at signup would be told to add a PIN to gain authority, which is a worse account offered as a security step. So the ceremony reuses the handoff factor enrollment already ships: `POST /security/authority/step-up/start` mints a one-time URL for one purpose, the client opens it in an `ASWebAuthenticationSession` or a Custom Tab, `gua-idp-web` runs the ceremony at the `AUTHORITY_STEP_UP` step, and the sheet closes back into the app through the app's own scheme.

What the sheet leaves behind is a row in `account_authority_web_step_up`, not a token the client carries: a proof this service wrote about a ceremony it ran, bound to the account, to the SHA-256 of the access token that asked for the sheet and to one purpose, spendable once, inside the challenge's own 15 minutes. `POST /account/authority/challenge` spends it, and only for a request that produced no assertion and no PIN of its own, so a client that can sign its own assertions never touches this path. Every control of the native step-up applies: the same two factors and no third, the fresh-factor hold weighed on what the sheet observed, the refusal while a recent account recovery is inside that hold, and the native-session rule, so a browser cannot open a sheet to arrange its own authority proof. The page has two arms and no OTP arm, which `AccountAuthorityGuardTest` pins on the method bodies and on the set of paths the class publishes, because that class legitimately holds the OTP services for signing in.
